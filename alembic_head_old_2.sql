...
-- Running upgrade 929afc1b3223 -> de6e635935c2
...
UPDATE alembic_version SET version_num='de6e635935c2' WHERE alembic_version.version_num = '929afc1b3223';

-- Running upgrade de6e635935c2 -> f5cccd3ee090
...
CREATE UNIQUE INDEX ix_history_log_id ON history_log (id);

CREATE INDEX ix_history_log_object_id ON history_log (object_id);

CREATE UNIQUE INDEX ix_history_log_pid ON history_log (pid);

UPDATE alembic_version SET version_num='f5cccd3ee090' WHERE alembic_version.version_num = 'de6e635935c2';

-- Running upgrade f5cccd3ee090 -> 2530b7439876
...
CREATE TABLE api_keys (
    ...
    modified_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL, 
    PRIMARY KEY (id), 
    FOREIGN KEY(app_id) REFERENCES apps (pid), 
    FOREIGN KEY(created_by) REFERENCES auth_users (pid), 
    FOREIGN KEY(organisation_id) REFERENCES organisations (pid), 
    UNIQUE (key)
);

CREATE UNIQUE INDEX ix_api_keys_id ON api_keys (id);

CREATE INDEX ix_api_keys_org_app_active ON api_keys (organisation_id, app_id, is_active);

CREATE UNIQUE INDEX ix_api_keys_pid ON api_keys (pid);

UPDATE alembic_version SET version_num='2530b7439876' WHERE alembic_version.version_num = 'f5cccd3ee090';

-- Running upgrade 2530b7439876 -> e6e640370561

ALTER TABLE api_keys ADD CONSTRAINT uq_api_keys_name_org_app UNIQUE (name, organisation_id, app_id);

UPDATE alembic_version SET version_num='e6e640370561' WHERE alembic_version.version_num = '2530b7439876';
